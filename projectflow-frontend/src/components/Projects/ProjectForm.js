...
const formatDateForInput = (dateString) => {
  ...
  // Safely create a Date object and format it
...
const ProjectForm = ({ isEditMode = false }) => {
  ...
  // We now get the `fetchUserProjects` function from the context.
  ...
  // useCallback is used here to memoize the function, preventing unnecessary re-renders.
  const fetchFormData = useCallback(async () => {
    try {
      setLoading(true);
      // Fetch the list of users who can be assigned to the project.
      const usersData = await projectService.getAssignableUsers();
      setAssignableUsers(usersData || []);

      // If in edit mode, fetch the existing project's data.
      if (isEditMode && projectId) {
        ...
        // For new projects, pre-populate the creator and manager if eligible.
  ...
  const handleSubmit = async (e) => {
    ...
    // Ensure the selected project manager is always included in the team members list.
    const finalFormData = {
      ...formData,
    ...
    try {
      ...
      // Navigate to the appropriate page after the action.
  ...
  if (loading) return <div className="form-loading"><p>Loading form...</p></div>;
    ...
    <div className="form-page-container">
      <div className="project-form-card">
        ...
        <form onSubmit={handleSubmit}>
          {/* Omitted the full form JSX for brevity, as it remains the same */}
